  ...
  <table class="form-table">
	<tr>
		<td class="d-grid">
			...
			<?php if ( License::license_valid() ) : ?>
				<span class="mb-2"><?php echo sprintf( esc_html__( 'Tipo da licença: %s', 'wc-account-genius' ), License::license_title() ) ?></span>
				<span class="mb-2"><?php echo sprintf( esc_html__( 'Licença expira em: %s', 'wc-account-genius' ), License::license_expire() ) ?></span>
				
				<span class="mb-2"><?php echo esc_html__( 'Sua chave de licença:', 'wc-account-genius' ) ?>
					<?php if ( ! empty( get_option('wc_account_genius_license_key') ) ) :
						echo esc_attr( substr( get_option('wc_account_genius_license_key'), 0, 9 ) . "XXXXXXXX-XXXXXXXX" . substr( get_option('wc_account_genius_license_key'), -9 ) );
					else :
						echo esc_html__(  'Não disponível', 'wc-account-genius' );
					endif; ?>
				</span>
			<?php endif; ?>
		</td>
